...
function seedBarcodeFood(): void {
  ...
  db.prepare('DELETE FROM foods WHERE barcode IN (?, ?)').run(KNOWN_BARCODE, UNKNOWN_BARCODE)
...
async function installCameraTracking(page: Page) {
  await page.addInitScript(() => {
    ...
    media.getUserMedia = async (...args: Parameters<typeof media.getUserMedia>) => {
...
test.describe('Barcode scanning (issue #58)', () => {
  ...
  test('manual fallback logs a known barcode in three taps', async ({ page }) => {
    await installManualBarcodeFallback(page)
    await openNutritionOn(page, TARGET_DATE)

    await clickHydratedButton(page.getByRole('button', { name: 'Scan barcode' }))
    const dialog = page.getByRole('dialog')
    await expect(dialog).toBeVisible()
    await expect(dialog.getByRole('textbox', { name: 'Barcode' })).toBeVisible()

    await dialog.getByRole('textbox', { name: 'Barcode' }).fill(KNOWN_BARCODE)
    await clickHydratedButton(dialog.getByRole('button', { name: 'Look up barcode' }))
    ...
    await expect(page.getByRole('cell', { name: KNOWN_FOOD_NAME }).first()).toBeVisible()
  })

  test('unknown barcode offers creation prefilled with the code', async ({ page }) => {
    await installManualBarcodeFallback(page)
    await openNutritionOn(page, TARGET_DATE)

    await clickHydratedButton(page.getByRole('button', { name: 'Scan barcode' }))
    const dialog = page.getByRole('dialog')
    await dialog.getByRole('textbox', { name: 'Barcode' }).fill(UNKNOWN_BARCODE)
    await clickHydratedButton(dialog.getByRole('button', { name: 'Look up barcode' }))
    await clickHydratedButton(dialog.getByRole('button', { name: 'Add this food' }))

    await expect(page.getByRole('heading', { name: 'New Custom Food', level: 3 })).toBeVisible()
    await expect(page.getByRole('textbox', { name: 'Barcode' })).toHaveValue(UNKNOWN_BARCODE)
  })

  test('requests camera permission only after Scan is tapped', async ({ page }) => {
    await installCameraTracking(page)
    await openNutritionOn(page, TARGET_DATE)

    await expect
      .poll(() => page.evaluate(() => (window as unknown as { __getUserMediaCalls?: number }).__getUserMediaCalls ?? 0))
      .toBe(0)
