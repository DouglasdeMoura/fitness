...
function breakfastSection(page: Page) {
  return page.getByRole('heading', { name: 'Breakfast', level: 3 }).locator('xpath=ancestor::*[2]')
}
...
test.describe('Quick add calories (issue #57)', () => {
  ...
  test('quick add is reachable in one tap from the nutrition page', async ({ page }) => {
    await openNutritionOn(page, TARGET_DATE)
    const quickAddButton = breakfastSection(page).getByRole('button', { name: 'Quick add' })
    await clickHydratedButton(quickAddButton)
    await expect(page.getByRole('dialog')).toBeVisible()
    await expect(page.getByText('Quick add — Breakfast')).toBeVisible()
  })

  test('logs approximate calories with badge and updates daily summary', async ({ page }) => {
    await openNutritionOn(page, TARGET_DATE)

    await clickHydratedButton(breakfastSection(page).getByRole('button', { name: 'Quick add' }))
    const dialog = page.getByRole('dialog')
    await dialog.getByLabel('Name').fill(QUICK_ADD_NAME)
    await dialog.getByRole('spinbutton', { name: 'Calories Required' }).fill(String(QUICK_ADD_CALORIES))
    await dialog.getByRole('spinbutton', { name: 'Protein (g) Optional' }).fill('25')
    await clickHydratedButton(dialog.getByRole('button', { name: 'Log quick add' }))

    await expect(page.getByText('Food logged', { exact: true })).toBeVisible()
    await expect(page.getByRole('cell', { name: `${QUICK_ADD_NAME} Approximate` })).toBeVisible()
    await expect(page.getByRole('cell', { name: String(QUICK_ADD_CALORIES) })).toBeVisible()
    await expect(
      page.getByRole('heading', { name: 'Daily Summary', level: 2 }).locator('xpath=ancestor::*[1]'),
    ).toContainText(String(QUICK_ADD_CALORIES))
    await expect(page.getByRole('cell', { name: /25 \/ 0 \/ 0 g/ })).toBeVisible()
  })
})
