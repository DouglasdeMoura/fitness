...
test.describe("Empty states with call-to-action buttons", () => {
  test("food log empty state focuses the search input", async ({ page }) => {
    ...
    const emptyState = page
      .getByRole("status")
      .filter({ hasText: "No food logged yet" });
    const addMealButton = page.getByRole("button", {
      exact: true,
      name: "Add your first meal",
    });
  ...
  test("food search empty state opens the custom food editor", async ({
    page,
  }) => {
    await openAppPage(page, "/nutrition");
    const searchInput = page.getByRole("textbox", { name: "Search foods" });
    ...
    await clickHydratedButton(
      page.getByRole("button", { exact: true, name: "Create a custom food" })
    );
    await expect(page.getByLabel("Name")).toBeVisible();
  });

  test("progress weight empty state navigates to settings", async ({
    page,
  }) => {
    ...
    const logWeightButton = page.getByRole("link", {
      exact: true,
    ...
    if (await emptyState.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      test.info().annotations.push({
        description:
          "Weight history already has data; empty-state CTA not shown.",
        type: "note",
      });
    }
  });
  ...
  test("workout sessions empty state starts a session", async ({ page }) => {
    ...
    const emptyState = page
      .getByRole("status")
      .filter({ hasText: "No workouts yet" });
    ...
    if (await emptyState.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      await expect(
        page.locator('.card-title:has-text("Exercise")')
      ).toBeVisible({
        timeout: 10_000,
      });
    } else {
      test.info().annotations.push({
        description:
          "Workout sessions already exist; empty-state CTA not shown.",
        type: "note",
      });
    }
  });
