...
test.describe("Focused session interface (issue #32)", () => {
  ...
  test("hides page heading and date navigation during active session", async ({
    page,
  }) => {
    ...
    await expect(
      page.getByRole("heading", { level: 1, name: "Workout" })
    ).not.toBeVisible();
    ...
    await expect(
      page
        ...
        .or(page.getByRole("button", { disabled: true, name: "Next day" }))
    ).not.toBeVisible();
  ...
  test("shows exercise name as display heading in focused view", async ({
    page,
  }) => {
    await startWorkout(page);
    ...
    const fontSize = await exerciseHeading.evaluate(
  ...
  test("shows collapsible stats panel after logging a set", async ({
    page,
  }) => {
    await startWorkout(page);
    await selectExercise(page, BENCH_PRESS_OPTION);
  ...
  test("rest timer auto-starts after saving a set with RPE-based duration", async ({
    page,
  }) => {
    await startWorkout(page);
    await selectExercise(page, BENCH_PRESS_OPTION);
  ...
  test("finish button is reachable at all times during active session", async ({
    page,
  }) => {
    await startWorkout(page);
