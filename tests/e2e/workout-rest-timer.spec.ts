...
test.describe('Rest timer (issue #60)', () => {
  ...
  test('starts on set log with RPE-based suggested duration', async ({ page }) => {
  ...
  test('survives in-app navigation away and back mid-rest', async ({ page }) => {
    await startWorkout(page)
    await selectExercise(page, BENCH_PRESS_OPTION)
    await page.getByRole('button', { name: 'Add set' }).click()
    await setSetValues(page, 80, 8, 7)
    await clickHydratedButton(page.getByRole('button', { name: /Save set 1/ }))
    await expect(restTimerRegion(page).getByText('2:00')).toBeVisible({ timeout: 10000 })
  ...
  test('fires a toast when rest completes', async ({ page }) => {
    ...
    await expect(page.getByRole('region', { name: 'Notifications' }).getByText('Rest complete')).toBeVisible({
      timeout: 10000,
    })
  })

  test('hides countdown progress animation under prefers-reduced-motion', async ({ page }) => {
    ...
    await clickHydratedButton(page.getByRole('button', { name: /Save set 1/ }))
    await expect(restTimerRegion(page).getByText('3:00')).toBeVisible({ timeout: 10000 })

    await expect(page.getByRole('progressbar', { name: 'Rest progress' })).toHaveCount(0)
    const offenders = await findReducedMotionOffenders(page)
    expect(offenders, offenders.join('\n')).toEqual([])
  })

  test('timer controls meet the 44px touch target minimum', async ({ page }) => {
    await startWorkout(page)
    await selectExercise(page, BENCH_PRESS_OPTION)
    await page.getByRole('button', { name: 'Add set' }).click()
    await setSetValues(page, 70, 8, 8)
    await clickHydratedButton(page.getByRole('button', { name: /Save set 1/ }))
    await expect(restTimerRegion(page)).toBeVisible({ timeout: 10000 })
