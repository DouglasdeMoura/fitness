...
test.describe("Gym-grade mobile ergonomics (issue #53)", () => {
  ...
  test("programs list scrolls inside its host without document overflow", async ({
    page,
  }) => {
    await openAppRoute(page, "/workout/programs");
    await assertTableScrollsInHost(
      page,
      NAMED_TABLE_SCROLL_LABELS.programsList
    );
  });

  test("templates list scrolls inside its host without document overflow", async ({
    page,
  }) => {
    await openAppRoute(page, "/nutrition/templates");
    await assertTableScrollsInHost(
      page,
      NAMED_TABLE_SCROLL_LABELS.templatesList
    );
  });

  test("food log scrolls inside its host after logging food", async ({
    page,
  }) => {
  ...
  test("workout sets use decimal/numeric input modes and 2.5 kg / 1 rep steppers", async ({
    page,
  }) => {
  ...
  test("destructive controls keep at least 8px from neighbors on workout sets", async ({
    page,
  }) => {
    await ensureActiveWorkoutWithExercise(page);
