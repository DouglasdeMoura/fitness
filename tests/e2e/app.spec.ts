...
test.describe("Dashboard - User Landing Experience", () => {
  ...
  test("displays calorie ring and remaining summary on first visit", async ({
    page,
  }) => {
    await waitForAppReady(page);
    ...
    await expect(
      page.locator("text=/kcal (remaining|over target)/")
    ).toBeVisible();
  });

  test("shows macro tracking section with protein, carbs, and fat", async ({
    page,
  }) => {
    await waitForAppReady(page);
    ...
    if (await welcome.isVisible({ timeout: 3000 }).catch(() => false)) {
      test.info().annotations.push({
        description:
          "First-time welcome state shown; dashboard data not yet available.",
        type: "note",
      });
      return;
    }
    ...
    await expect(
      page.getByRole("progressbar", { name: "Protein consumed" })
    ).toBeVisible();
    await expect(
      page.getByRole("progressbar", { name: "Carbs consumed" })
    ).toBeVisible();
    await expect(
      page.getByRole("progressbar", { name: "Fat consumed" })
    ).toBeVisible();
  });

  test("renders quick actions as prominent clickable cards", async ({
    page,
  }) => {
    await waitForAppReady(page);
    const welcome = page
    ...
    if (await welcome.isVisible({ timeout: 3000 }).catch(() => false)) {
      test.info().annotations.push({
        description:
          "First-time welcome state shown; quick actions not yet available.",
        type: "note",
      });
      return;
    }
  ...
  test("shows current goal type from user profile", async ({ page }) => {
    ...
    if (await welcome.isVisible({ timeout: 3000 }).catch(() => false)) {
      test.info().annotations.push({
        description:
          "First-time welcome state shown; goal summary not yet available.",
        type: "note",
      });
      return;
    }
  ...
  test("quick action cards navigate to their target page", async ({ page }) => {
    ...
    await page
      .getByRole("link", { name: "Log your meals" })
      .click({ force: true });
...
test.describe("Nutrition - Food Logging Flow", () => {
  ...
  test("food search streams results live as the user types", async ({
    page,
  }) => {
    await openAppPage(page, "/nutrition");
  ...
  test("user can open custom food form from an unsuccessful food search", async ({
    page,
  }) => {
    await openAppPage(page, "/nutrition");
    const searchInput = page.getByRole("textbox", { name: "Search foods" });
    ...
    const noResults = page
      .getByRole("status")
      .filter({ hasText: "No foods found" });
    await expect(noResults).toBeVisible({ timeout: 10_000 });

    await page
      .getByRole("button", { exact: true, name: "Create a custom food" })
      .click();
  ...
  test("repeat food logs from Recent in at most two taps", async ({ page }) => {
    ...
    const recentFood = page.getByRole("button", {
      name: new RegExp(`^${foodName} —`),
    });
...
test.describe("Workout - Session Logging Flow", () => {
  ...
  test("shows start workout prompt when no active session", async ({
    page,
  }) => {
  ...
  test("user can start a workout session and see exercise selection", async ({
    page,
  }) => {
    await openAppPage(page, "/workout");
  ...
  test("selecting an exercise shows set logging interface", async ({
    page,
  }) => {
    await openAppPage(page, "/workout");
    const startBtn = page.getByRole("button", { name: "Start Workout" });
...
test.describe("Settings - Profile Configuration", () => {
  ...
  test("displays goal options with science-based descriptions", async ({
    page,
  }) => {
    await openAppPage(page, "/settings");
    ...
    await expect(
      listbox.getByRole("option", { name: /Build Muscle.*surplus/ })
    ).toBeVisible();
    await expect(
      listbox.getByRole("option", { name: /Lose Fat.*deficit/ })
    ).toBeVisible();
    await page.keyboard.press("Escape");
  });

  test("user can edit profile name via TanStack Form and save", async ({
    page,
  }) => {
    await openAppPage(page, "/settings");
  ...
  test("user can change activity level and save profile", async ({ page }) => {
    ...
    const listbox = page.getByRole("listbox");
    await expect(
      listbox.getByRole("option", { name: /Sedentary/ })
    ).toBeVisible();
  ...
  test("shows weight logging interface and accepts a weigh-in", async ({
    page,
  }) => {
    await openAppPage(page, "/settings");
    await expect(
  ...
  test("user can export data as JSON", async ({ page }) => {
    ...
    expect(download.suggestedFilename()).toMatch(
      /^fittrack-export-\d{4}-\d{2}-\d{2}\.json$/
    );
  });
});

test.describe("Progress - Analytics View", () => {
  test("user can navigate to progress page and see its heading", async ({
    page,
  }) => {
    await waitForAppReady(page);
  ...
  test("renders the highlights card with best lift, monthly volume, and streak", async ({
    page,
  }) => {
    await openAppPage(page, "/progress");
    await expect(
      page.getByText("Best Lift This Month", { exact: true })
    ).toBeVisible({ timeout: 10_000 });
    await expect(
      page.getByText("Monthly Volume", { exact: true })
    ).toBeVisible();
    await expect(
      page.getByText("Workout Streak", { exact: true })
    ).toBeVisible();
  });

  test("shows TabList for Weight, Volume, Nutrition views", async ({
    page,
  }) => {
    await openAppPage(page, "/progress");
    await expect(page.locator("button", { hasText: "Weight" })).toBeVisible({
      timeout: 10_000,
    });
    await expect(page.locator("button", { hasText: "Volume" })).toBeVisible();
    await expect(
      page.locator("button", { hasText: "Nutrition" })
    ).toBeVisible();
  });

  test("Weight tab defaults to area chart or empty guidance", async ({
    page,
  }) => {
    await openAppPage(page, "/progress");
    ...
    const empty = page
      .getByRole("status")
      .filter({ hasText: "No weight logs yet" });
    await expect(chart.or(empty).first()).toBeVisible({ timeout: 10_000 });
  });

  test("Volume tab shows Schoenfeld reference after switching", async ({
    page,
  }) => {
    await openAppPage(page, "/progress");
  ...
  test("Nutrition tab shows weekly averages after switching", async ({
    page,
  }) => {
    await openAppPage(page, "/progress");
    await page.locator("button", { hasText: "Nutrition" }).click();
    await expect(page.getByText(/Weekly Nutrition Summary/)).toBeVisible({
      timeout: 5000,
    });
  });

  test("logged workout volume renders as a ProgressBar on the Volume tab", async ({
    page,
  }) => {
    ...
    await page
      .getByRole("button", { name: /^Save set \d+$/ })
      .first()
      .click();
    await page.waitForTimeout(500);
...
test.describe("Dark Mode Toggle", () => {
  ...
  test("clicking toggle changes theme attribute via Astryx Theme", async ({
    ...
    await expect
      .poll(async () =>
        page.evaluate(() => localStorage.getItem("fittrack-theme"))
      )
      .toBe("dark");
  });
...
test.describe("AppShell TopNav selection", () => {
  test("marks the current section as selected for screen readers", async ({
    page,
  }) => {
    await waitForAppReady(page);
    ...
    await expect(page).toHaveURL(/\/nutrition/);
    await expect(
      nav(page).getByRole("link", { name: "Nutrition" })
    ).toHaveAttribute("aria-current", "page");
    await expect(
      nav(page).getByRole("link", { name: "Dashboard" })
