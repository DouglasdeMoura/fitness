...
export async function measureHeroMetricRatio(page: Page): Promise<HeroMetricRatio | null> {
  return page.evaluate(
    ({ minRatio }) => {
      ...
      const heroes = [...document.querySelectorAll('main [data-size]')].filter((element) => {
        const text = element.textContent?.trim() ?? ''
        const size = element.getAttribute('data-size')
        const tag = element.tagName.toLowerCase()
        if (!size || !heroSizes.has(size) || tag === 'input' || tag === 'button') {
          return false
        }
        return /\d/.test(text)
      })
      ...
      let best: { text: string; heroPx: number; bodyPx: number; ratio: number } | null = null
      ...
      for (const element of heroes) {
        ...
        const token = getComputedStyle(element).getPropertyValue(`--font-size-${size}`).trim()
        if (!token) {
          continue
        }
...
export async function findNonTokenTransitionElements(page: Page): Promise<string[]> {
  return page.evaluate(
    ({ toleranceMs, tokens }) => {
      const isNearToken = (ms: number) =>
        ms === 0 || tokens.some((token: number) => Math.abs(token - ms) <= toleranceMs)
    ...
    { toleranceMs: DURATION_TOLERANCE_MS, tokens: [...ASTRYX_DURATION_MS] },
  )
}

export async function findReducedMotionOffenders(page: Page): Promise<string[]> {
  return page.evaluate(() => {
    const offenders: string[] = []
...
export async function measureLowBodyContrastSamples(page: Page): Promise<BodyContrastSample[]> {
  return page.evaluate((minContrast) => {
    ...
    const low: Array<{ text: string; ratio: number }> = []
    ...
    for (const element of document.querySelectorAll(
      ...
      let background: [number, number, number] | null = parseRgb(style.backgroundColor)
