...
test.describe('Settings page (issue #34)', () => {
  ...
  test('renders grouped Card sections with Heading level={3} headings', async ({ page }) => {
    const main = page.getByRole('main')
    await expect(main.getByRole('heading', { name: 'Profile', level: 3 })).toBeVisible()
    await expect(main.getByRole('heading', { name: 'Goals', level: 3 })).toBeVisible()
    await expect(main.getByRole('heading', { name: 'Body Metrics', level: 3 })).toBeVisible()
    await expect(main.getByRole('heading', { name: 'Data Management', level: 3 })).toBeVisible()
    await expect(main.getByRole('heading', { name: 'About', level: 3 })).toBeVisible()
  })

  test('goal selector renders as visual SelectableCard grid', async ({ page }) => {
  ...
  test('activity level uses SegmentedControl', async ({ page }) => {
    ...
    await expect(segmented.getByRole('radio', { name: 'Sedentary' })).toBeVisible()
    await expect(segmented.getByRole('radio', { name: 'Lightly active' })).toBeVisible()
    await expect(segmented.getByRole('radio', { name: 'Moderately active' })).toBeVisible()
    await expect(segmented.getByRole('radio', { name: 'Very active' })).toBeVisible()
    await expect(segmented.getByRole('radio', { name: 'Extra active' })).toBeVisible()
  })
  ...
  test('data management section has export and import buttons', async ({ page }) => {
