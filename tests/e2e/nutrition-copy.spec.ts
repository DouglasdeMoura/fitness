...
function seedYesterdayFoodLog(): void {
  ...
  if (!user || !food) {
    db.close()
    throw new Error('Expected seeded user and Chicken Breast food for nutrition copy e2e')
  }
...
test.describe('Copy yesterday (issue #55)', () => {
  ...
  test('logs a full repeated day in one tap', async ({ page }) => {
    ...
    let taps = 0
    const copyDay = page.getByRole('button', { name: 'Copy yesterday', exact: true })
    await expect(copyDay).toBeVisible()
    await clickHydratedButton(copyDay)
    taps += 1

    await expect(page.getByRole('heading', { name: 'Breakfast', level: 3 })).toBeVisible()
    await expect(breakfastFoodRows(page)).toHaveCount(2)
    await expect(page.getByRole('heading', { name: 'Lunch', level: 3 })).toBeVisible()
  ...
  test('copy toast undo removes every copied entry', async ({ page }) => {
    ...
    await clickHydratedButton(page.getByRole('button', { name: 'Copy yesterday', exact: true }))
    ...
    await expect(page.getByRole('button', { name: 'Copy yesterday', exact: true })).toBeVisible()
    await expect(breakfastFoodRows(page)).toHaveCount(0)
  })

  test('meal copy action is hidden when it would be a no-op', async ({ page }) => {
    await installDeterministicClock(page)
    await openNutritionOn(page, TARGET_DATE)

    await expect(
      page.getByRole('button', { name: 'Copy breakfast from yesterday', exact: true }),
    ).toBeVisible()
    await expect(
      page.getByRole('button', { name: 'Copy lunch from yesterday', exact: true }),
    ).toBeVisible()
