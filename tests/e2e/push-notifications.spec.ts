...
async function ensurePushSubscribed(page: Page): Promise<void> {
  ...
  await expect(enableButton).toBeVisible({ timeout: 15000 })
...
test.describe('Web Push in Settings (issue #65)', () => {
  test('does not request notification permission before the user enables push', async ({
    ...
    await page.addInitScript(() => {
      ...
      ;(window as Window & { __fittrackPermissionRequested?: boolean }).__fittrackPermissionRequested =
        false
    ...
    await expect(page.getByRole('heading', { name: PUSH_CARD_TITLE, level: 2 })).toBeVisible()
  ...
  test('subscribed client receives a test push notification', async ({ browser }) => {
    ...
    await expect(page.getByText(PUSH_TEST_SUCCESS_MESSAGE)).toBeVisible({ timeout: 15000 })
  ...
  test('shows install guidance on iOS Safari before installation', async ({ page }) => {
    await page.addInitScript(() => {
    ...
    await openSettings(page)
    await expect(page.getByText(/add FitTrack to your Home Screen/i)).toBeVisible()
  })
})
