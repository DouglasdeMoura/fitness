...
for (const route of MIGRATED_ROUTES) {
  test.describe(`design gates on ${route}`, () => {
    ...
    test('hero metric font-size is at least 2.5× body text (numbers are heroes)', async ({ page }) => {
    ...
    test('top-level page sections have generous spacing (>= 24px)', async ({ page }) => {
      await prepareTheme(page, 'light')
      await openAppRoute(page, route)
    ...
    test('motion uses Astryx duration tokens in page content', async ({ page }) => {
      await prepareTheme(page, 'light')
      await openAppRoute(page, route)
    ...
    for (const colorMode of COLOR_MODES) {
      test(`body text contrast is at least 4.5:1 (${colorMode})`, async ({ page }) => {
        ...
        expect(
          lowContrast,
          lowContrast.map((sample) => `"${sample.text}" = ${sample.ratio}:1`).join('\n'),
        ).toEqual([])
      })
    }
  })
...
test.describe('reduced motion (issue #50)', () => {
  for (const route of MIGRATED_ROUTES) {
    test(`${route} disables transitions when prefers-reduced-motion is set`, async ({ page }) => {
      await prepareTheme(page, 'light')
