...
test.describe('Delete confirmation dialogs (issue #25)', () => {
  test('food log delete requires dialog confirmation and Cancel keeps the entry', async ({
    ...
    await expect(dialog.getByRole('heading', { name: 'Delete this entry?' })).toBeVisible()
    await clickHydratedButton(dialog.getByRole('button', { name: 'Cancel delete' }))
    await expect(dialog).not.toBeVisible()
    await expect(foodRow).toBeVisible()
  })

  test('workout set delete shows dialog with destructive confirm button', async ({ page }) => {
    ...
    const dialog = deleteDialog(page)
    await expect(dialog).toBeVisible()
    await expect(dialog.getByRole('heading', { name: 'Delete this set?' })).toBeVisible()
    ...
    const secondaryButton = dialog.getByRole('button', { name: 'Cancel delete' })
  ...
  test('program delete from list shows named confirmation dialog', async ({ page }) => {
    ...
    const dialog = deleteDialog(page)
    await expect(dialog).toBeVisible()
    await expect(dialog.getByRole('heading', { name: `Delete '${match![1]}'?` })).toBeVisible()
