...
async function errorTokenRgb(
  page: import("@playwright/test").Page
): Promise<string> {
  return page.evaluate(() => {
    const probe = document.createElement("span");
    probe.className = "astryx-statusdot";
    probe.dataset.variant = "error";
...
test.describe("Dashboard consistency tracking", () => {
  ...
  test("missed days use neutral status, not the error colour token", async ({
    page,
  }) => {
    const card = page.getByLabel("Consistency tracking");
    const errorRgb = await errorTokenRgb(page);
    const missedDots = card.locator(
      '.astryx-statusdot[data-variant="neutral"]'
    );
    ...
    for (let index = 0; index < missedCount; index++) {
      ...
      const dotRgb = await dot.evaluate(
    ...
    await expect(
      card.locator('.astryx-statusdot[data-variant="error"]')
    ).toHaveCount(0);
  });
});
