...
test.describe('Session summary on finish (issue #62)', () => {
  ...
  test('finish workout shows summary with volume, sets, exercises, duration, and PRs', async ({ page }) => {
  ...
  test('summary is reachable again from workout history', async ({ page }) => {
    ...
    await page.getByRole('link', { name: /View summary Training Session/i }).first().click()
    await expect(page.getByRole('heading', { name: 'Session Summary' })).toBeVisible()
    await expect(page.getByLabel('Session volume comparison')).toContainText(/kg total/)
  })
})
