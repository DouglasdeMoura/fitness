...
test.describe('Keyboard shortcuts (issue #35)', () => {
  ...
  test('? opens the shortcuts help dialog and shows kbd elements', async ({ page }) => {
    test.skip(test.info().project.name !== 'chromium', 'Keyboard shortcuts only on desktop')
  ...
  test('n key triggers food log dialog on nutrition page', async ({ page }) => {
    ...
    const buttonIsVisible = await logFoodButton.isVisible({ timeout: 5000 }).catch(() => false)

    if (!buttonIsVisible) {
      test.skip(true, 'Log food button not visible on this page (may need seed data)')
      return
    }
    ...
    await expect(page.getByRole('dialog', { name: 'Log food' })).toBeVisible({ timeout: 10000 })
  })
})
