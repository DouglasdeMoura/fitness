...
async function resolveFreshWeight(page: Page): Promise<number> {
  ...
  await expect(lastTime.or(page.getByText(NO_HISTORY_GUIDANCE))).toBeVisible({ timeout: 10000 })

  if (await lastTime.isVisible()) {
    const text = await lastTime.textContent()
    const match = text?.match(/Last time: ([\d.]+) kg/)
    if (match) {
      return Math.round(parseFloat(match[1]) + 5)
    }
  }
  return 205
}

async function setSetValues(page: Page, setNumber: number, weight: number, reps: number, rpe: number) {
  const row = page
...
test.describe('Personal record detection (issue #61)', () => {
  ...
  test('detects rep PR on save, badges the set, and shows PR in session history', async ({
    ...
    await expect(toastRegion(page).getByRole('status').filter({ hasText: 'Set saved' })).toBeVisible({
      timeout: 10000,
    })
    await expect(toastRegion(page).getByRole('status').filter({ hasText: /PR/ })).toHaveCount(0)
    ...
    const viewSession = page.getByRole('link', { name: /View session/i }).first()
    await expect(viewSession).toBeVisible({ timeout: 10000 })
    ...
    await expect(
      page.getByRole('table', { name: 'Logged sets for this session' }).getByText('PR').first(),
    ).toBeVisible({ timeout: 10000 })
  })
})
