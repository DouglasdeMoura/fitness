...
test.describe("Loading states", () => {
  test("nutrition page shows a skeleton while data for a new date is loading", async ({
    page,
  }) => {
    ...
    await clickHydratedButton(
      page.getByRole("main").getByRole("button", { name: "Previous day" })
    );
  ...
  test("food search shows a spinner while the debounced query is pending", async ({
    page,
  }) => {
    await openAppPage(page, "/nutrition");
    ...
    await expect(
      page.getByRole("status", { name: "Searching foods" })
    ).toBeVisible();
  });

  test("profile save button shows a loading state while submitting", async ({
    page,
  }) => {
    let release!: () => void;
    ...
    await page.route("**/*", async (route) => {
      ...
      if (
        request.method() === "POST" &&
        request.postData()?.includes("updateUser")
      ) {
        await gate;
      }
      await route.continue();
