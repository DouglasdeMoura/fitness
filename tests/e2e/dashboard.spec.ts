...
test.describe('Dashboard calorie ring (issue #30)', () => {
  ...
  test('renders SVG calorie ring with accessible label', async ({ page }) => {
    ...
    const ring = page.locator('svg[role="img"][aria-label*="Calorie progress"]')
    const welcome = page.getByRole('status').filter({ hasText: 'Welcome to FitTrack' })

    const ringVisible = await ring.isVisible({ timeout: 3000 }).catch(() => false)
    const welcomeVisible = await welcome.isVisible({ timeout: 3000 }).catch(() => false)
  ...
  test('hero number uses large typography (at least 2xl token)', async ({ page }) => {
    await prepareTheme(page, 'light')
    await waitForDashboardReady(page)
  ...
  test('units and labels use supporting text style', async ({ page }) => {
    ...
    if (await welcome.isVisible({ timeout: 3000 }).catch(() => false)) {
      test.info().annotations.push({
        type: 'note',
        description: 'First-time welcome state shown; supporting text test skipped.',
      })
      return
    }
  ...
  for (const colorMode of COLOR_MODES) {
    test(`calorie ring track and fill use Astryx tokens (${colorMode})`, async ({ page }) => {
      ...
      if (await welcome.isVisible({ timeout: 3000 }).catch(() => false)) {
        test.info().annotations.push({
          type: 'note',
          description: 'First-time welcome state shown; ring token test skipped.',
        })
        return
      }

      // The SVG ring has two circles: track and fill
      const circles = page.locator('svg[role="img"][aria-label*="Calorie progress"] circle')
      const count = await circles.count()
      expect(count).toBeGreaterThanOrEqual(2)

      // Track circle should use var(--color-track)
      const trackStroke = await circles.nth(0).evaluate((el) =>
...
test.describe('Dashboard first-time welcome state (issue #30)', () => {
  ...
  test('shows welcome prompt instead of zeros for new users', async ({ page }) => {
    await prepareTheme(page, 'light')
    await waitForDashboardReady(page)
...
test.describe('Dashboard quick actions (issue #30)', () => {
  ...
  test('quick actions render as ClickableCards with description text', async ({ page }) => {
    await prepareTheme(page, 'light')
    await waitForDashboardReady(page)
    ...
    if (await welcome.isVisible({ timeout: 3000 }).catch(() => false)) {
      test.info().annotations.push({
        type: 'note',
        description: 'First-time welcome state shown; quick action test skipped.',
      })
      return
    }
  ...
  test('quick action cards have accessible labels', async ({ page }) => {
    ...
    if (await welcome.isVisible({ timeout: 3000 }).catch(() => false)) {
      test.info().annotations.push({
        type: 'note',
        description: 'First-time welcome state shown; accessibility test skipped.',
      })
      return
    }
