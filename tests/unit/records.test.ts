...
const bench = (sessionId: number, weight: number, reps: number, id?: number): ExerciseSetSnapshot => ({
  id,
...
describe('detectPersonalRecords (issue #61)', () => {
  ...
  it('does not flag an exact 1RM tie as a PR', () => {
    ...
    expect(records.find((record) => record.kind === 'estimated_1rm')).toBeUndefined()
  })
  ...
  it('detects volume PR when the session total beats prior sessions', () => {
    ...
    expect(records).toContainEqual({ kind: 'volume', previousBest: priorVolume })
  })
  ...
  it('includes earlier sets from the same session when evaluating a new set', () => {
    ...
    const records = detectPersonalRecords(prior, bench(2, 100, 10, 2), [prior[0]])
...
describe('recordKindsBySetId', () => {
  it('maps chronological sets to the record kinds each one broke', () => {
    ...
    expect(kinds.get(2)).toEqual(expect.arrayContaining(['estimated_1rm', 'rep']))
