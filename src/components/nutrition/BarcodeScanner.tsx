...
import { addFoodLogEntry, getFoodByBarcode, getLoggedFoodStats } from '~/lib/api'
import {
...
import { buildFoodLogDraft, mealTypeForHour, type MealType } from '~/lib/nutrition'
...
async function defaultLogContext(foodId: number): Promise<{ mealType: MealType; servings: number }> {
  try {
 ...
 */
export function BarcodeScanner({ selectedDate, onSelectFood, onCreateFood }: BarcodeScannerProps) {
  ...
  const runLookup = useCallback(
    async (raw: string) => {
      ...
      if (!food) {
        setLookup({ status: 'missing', barcode: normalized })
        return
      }
      const context = await defaultLogContext(food.id)
      setLookup({ status: 'found', food, ...context })
    },
    [],
  )
  ...
  const startCamera = useCallback(async () => {
    ...
    try {
      ...
      setCameraError('Camera access was denied or is unavailable. Enter the barcode manually.')
    }
  ...
  return (
    <>
      <Button label="Scan barcode" variant="secondary" clickAction={openScanner} />
      <Dialog isOpen={isOpen} onOpenChange={handleOpenChange} purpose="form" width={360}>
        <DialogHeader
          title="Scan barcode"
          subtitle={
            detectorSupported
        ...
        <VStack gap={3}>
          {detectorSupported ? (
            <VStack gap={2}>
              ...
              />
              {cameraError ? <Text type="supporting">{cameraError}</Text> : null}
            </VStack>
          ) : null}
          <FormLayout>
            <TextInput
          ...
          <HStack gap={2} wrap="wrap">
            ...
            />
            <Button label="Cancel barcode scan" clickAction={() => handleOpenChange(false)} />
          </HStack>
          {lookup.status === 'looking' ? (
            <Text type="supporting">Looking up barcode…</Text>
          ) : null}
          {lookup.status === 'invalid' ? (
            <Text type="supporting">Enter a valid GTIN (8, 12, 13, or 14 digits).</Text>
          ) : null}
          {lookup.status === 'error' ? (
          ...
          {lookup.status === 'found' ? (
            <VStack gap={2}>
              ...
              <Text type="supporting">
                {lookup.food.calories_per_serving} kcal per {lookup.food.serving_size}
          ...
          {lookup.status === 'missing' ? (
            <VStack gap={2}>
              <Text type="supporting">
                No food saved for barcode {lookup.barcode}. Add it once and future scans will match.
              </Text>
              <Button
                label="Add this food"
                variant="primary"
...
export function matchCachedFoodBarcode(
  ...
  return foods.find((food) => food.barcode != null && variants.has(food.barcode)) ?? null
}
