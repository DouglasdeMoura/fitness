...
export type RunScheduledNotificationsInput = {
...
async function deliverTypeForUser(
  ...
  type: ScheduledNotificationType,
  slot: string,
): Promise<'delivered' | 'not_due' | 'duplicate' | 'no_subscription' | 'not_configured'> {
  ...
  await sendPushToUserSubscriptions(input.db, input.client, input.vapid, userId, payload)
  return 'delivered'
}
