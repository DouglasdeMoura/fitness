...
export type WeekNutritionMetrics = {
...
export type WeekTrainingMetrics = {
  totalVolume: number
  setCount: number
  sessionCount: number
...
export type DailyNutritionSlice = {
  calories: number
  protein_g: number
}
...
export function lastCompleteWeekRange(asOf: string): WeekRange {
  ...
  return { start: getWeekStart(end), end }
}
 ...
 */
export function hasReviewableWeek(asOf: string, activityDates: string[]): boolean {
...
function summarizeTrainingForWeek(
  ...
  const currentSets = sets.filter((set) => set.date >= range.start && set.date <= range.end)
...
export function generateWeeklyReviewHeadline(facts: WeeklyReviewFacts): string {
  ...
  const volumeUp = training.volumeDirection === 'more' && volumePct != null && volumePct >= 8
  const hasActivity =
    nutrition.loggedDays > 0 || training.sessionCount > 0 || personalRecordCount > 0
  ...
  if (personalRecordCount > 0) {
    const label = personalRecordCount === 1 ? 'personal record' : 'personal records'
...
export function assembleWeeklyReview(input: {
  ...
  const activityDates = [
    ...foodDates,
    ...input.sessionDates,
    ...input.bodyLogs.filter((log) => log.weight_kg != null).map((log) => log.date),
  ]
