    ...
    dbPromise = new Promise((resolve, reject) => {
      ...
      request.onupgradeneeded = () => {
        ...
        if (!db.objectStoreNames.contains(OUTBOX_STORE)) {
          const outbox = db.createObjectStore(OUTBOX_STORE, { keyPath: 'client_id' })
...
export async function refreshOfflineBundle(): Promise<OfflineBundle | null> {
  ...
  try {
    ...
    await request(KV_STORE, 'readwrite', (store) => store.put({ key: BUNDLE_KEY, value: bundle }))
    return bundle
  } catch {
...
export async function readOfflineBundle(): Promise<OfflineBundle | null> {
  if (!isBrowser()) return null
  try {
    const row = await request<{ key: string; value: OfflineBundle } | undefined>(
      KV_STORE,
      'readonly',
      (store) => store.get(BUNDLE_KEY)
    )
 ...
 */
export async function searchCachedFoods(query: string, limit = 20): Promise<OfflineBundle['foods']> {
...
export async function getCachedFoodLog(date: string): Promise<OfflineBundle['food_log']> {
  const bundle = await readOfflineBundle()
  if (!bundle) return []
...
export async function getQueuedMutations(): Promise<QueuedMutation[]> {
  if (!isBrowser()) return []
  try {
    const all = await request<QueuedMutation[]>(OUTBOX_STORE, 'readonly', (store) => store.getAll())
...
export async function getQueueSize(): Promise<number> {
  ...
  try {
    return await request<number>(OUTBOX_STORE, 'readonly', (store) => store.count())
  } catch {
    return 0
  }
}
...
async function recordFailure(clientId: string, message: string): Promise<void> {
  const existing = await request<QueuedMutation | undefined>(OUTBOX_STORE, 'readonly', (store) =>
    store.get(clientId)
  )
  if (!existing) return
 ...
 */
export async function syncOutbox(): Promise<{ applied: number; failed: number }> {
  if (!isBrowser() || syncing || !navigator.onLine) return { applied: 0, failed: 0 }
  ...
  try {
    ...
    const settled = result.outcomes
      .filter((outcome) => outcome.status === 'applied' || outcome.status === 'duplicate')
      .map((outcome) => outcome.client_id)
    await removeFromOutbox(settled)

    for (const outcome of result.outcomes) {
      if (outcome.status === 'failed') {
        await recordFailure(outcome.client_id, outcome.error ?? 'unknown error')
      }
    }
