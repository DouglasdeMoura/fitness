...
function expectedTrainingDays(windowDays: number, restDaysAllowed: number): number {
  if (windowDays <= 0) {
    throw new Error(
...
export function logAdherence(
  ...
  const daysWithLogs = windowDates(asOf, windowDays).filter((day) => logged.has(day)).length
...
export function workoutAdherence(
  ...
  if (windowDays <= 0) {
    throw new Error(`workoutAdherence: windowDays must be > 0, got ${windowDays}`)
  }
  ...
  const daysMet = windowDates(asOf, windowDays).filter((day) => sessions.has(day)).length
...
export function buildLast7Days(dates: string[], asOf: string): ConsistencyDay[] {
  const logged = new Set(dates)
